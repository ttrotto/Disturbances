...
![Illustration of the steps conducted to determine distance from infested trees to closest forest edge. The yellow square is the selected grid cell. The light blue arrow is the distance from the infested tree and the nearest forest edge. The zoomed in view helps you visualize steps 3 and 5. The green line is the distance from an infested tree to the nearest neighbor, while the pink line is the distance to a random point (the grid cell's center in our case).](data/webworm_analysis.png){#fig-sketch}
